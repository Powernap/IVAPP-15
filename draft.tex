...
%%
Furthermore, previous analyses mainly focused on a single parameter (e.g. \emph{BMI}) or a small subset of parameters.
%%
...
%%
\item The detection of correlations between image-based and socio-demographic as well as medical parameters for an improved hypothesis generation,
%%
...
%%
We combine both ideas by validating the significance of image-derived parameters.
...
%%
This yields a total of 9 image-derived parameters.
%%
...
%%
When correlating spine curvature and torsion with non-image parameter, all these variables need to be taken into account
%%
...
\end{figure*}
%%
\noindent As a first experiment we correlated the shape parameter with the binary back pain indicator using \texttt{GPLOM's} \cite{GPLOMS}.
%%
...
%%
The PCA results in a linear projection of the parameter space ordering their components by maximum variance.
%%
...
%%
As opposed to the standard \texttt{R} plots, \texttt{ggvis} allows to adjust visualization parameters using user interface controls, such as sliders.
%%
...
%%
  ...
  \label{fig:results}
\end{figure*}
\com{ToDo: Eigene Section daraus machen?}. 
In this section, we show which non-image variables can be described using the 9 image derived variables.
%%
...
%
The question is, whether the inter-group spine-variability parameter is enough for predicting other parameters or not.
%%
...
%%TODO HIER WEITER
%We carried out an analysis on subjects divided by \emph{gender}, \emph{BMI}, and \emph{body size} to assess their influence on the lumbar spine.
%To assess the influence of parameters which strongly influence the spine shape, we applied the technique to.
%%
